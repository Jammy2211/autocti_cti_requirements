...
input_model_parameters = [0.07275, 0.8, 0.21825, 4.0, 6.54804, 20.0, 0.58]
ylabels = [
    r"$\rho_{1}$",
    r"$\tau_{1}$",
    r"$\rho_{2}$",
...
plt.figure(figsize=(16.0, 19.0), dpi=100)
...
for param_index in range(total_params):
    ...
    subplot_index = [1, 3, 5, 7, 9, 11, 13]

    plt.subplot(7, 2, subplot_index[param_index])
    ...
    for image_index in range(total_images):
        ...
        eb = plt.errorbar(
            ...
            elinewidth=1,
            markeredgewidth=3,
            color=color_list[image_index],
            ecolor=color_list[image_index]
        )
    ...
    if subplot_index[param_index] == 13:
...
for param_index in range(total_params):
    ...
    subplot_index = [2, 4, 6, 8, 10, 12, 14]
    ...
    plt.gca().yaxis.set_major_formatter(StrMethodFormatter("{x:,.4f}"))
    plt.ylabel(ylabels[param_index], fontsize=12)
    if subplot_index[param_index] == 14:
        plt.xlabel("$EPER_{\mathrm{8}}$", usetex=True, fontsize=12)
...
plt.savefig(path.join(plot_path, "serial_x3_estimates.png"))
plt.show()
